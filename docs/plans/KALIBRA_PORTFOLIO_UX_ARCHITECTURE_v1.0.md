# Kalibra — Portfolio UX Architecture v1.0

**Status:** Recorded — architecture only. No screens, no HTML/CSS/JS, no implementation, no dependency change.
**Date:** 2026-07-07
...
**Audience:** Werkstudent recruiters and technically literate junior-hiring reviewers.
...
This document defines the **architecture of the recruiter experience** for Kalibra —
*what experience should exist before any pixel is designed.* It is not UI design, not
visual styling, not implementation. It commits no HTML, CSS, or JavaScript, adds no
dependency, and modifies no normative document (no ADR, Strategy, or roadmap).

Every experience proposed here is bounded to evidence that already exists in the
repository under `docs/evidence/`, `docs/checkpoints/`, and `artifacts/runtime/`.
Where Kalibra has not demonstrated a capability, this architecture requires the UI to
say so plainly rather than imply it. The architecture refuses any element that would
assert a capability Kalibra has not evidenced.

The immediately prior artifact —
[KALIBRA_PORTFOLIO_UX_STACK_AND_PROTOTYPE_REVIEW_v1.0](../checkpoints/KALIBRA_PORTFOLIO_UX_STACK_AND_PROTOTYPE_REVIEW_v1.0.md) —
established two facts that constrain everything below:

1. **Real engineering depth exists.** Governed VisA acquisition, PaDiM baseline, ONNX
   export with export-equivalence, runtime-equivalence, replay-verified runtime
   integration, and placeholder retirement — all hash-anchored and inspectable.
2. **Calibration does not exist yet.** The runtime carries a real *raw anomaly
   measure* but produces no calibrated confidence, no accept/review/reject outcome, no
   abstention, and no drift. This is the honest boundary of the experience.

These two facts — deep runtime, absent trust layer — define the entire envelope.
...
**Why this UI exists.** To let a recruiter who does *not* trust the system verify, in a
...
**What recruiters remember, by dwell time:**
...
**Single success test:** a technically literate recruiter who tries to poke a hole in
...
7. The Honest Boundary   →  What is NOT yet demonstrated (trust/calibration/drift)
        ↓
8. Engineering Timeline  →  Checkpoints as a governed build history
        ↓
...
- **The thesis (step 2) is placed early but after the hero**, so the recruiter has a
...
- **The honest boundary (step 7) comes *after* the strong results**, never before. A
  recruiter who has already seen real equivalence proofs reads "trust layer not yet
  built" as *discipline*; a recruiter who saw it first would read it as *incompleteness*.
- **Timeline is late (step 8), not early.** Process is only interesting once the
  recruiter believes the output is real.
- **Repository is the exit, not the entrance.** The UI earns the click to GitHub by
  first proving the work is worth reading.

The journey must be **skimmable in one axis**: a recruiter can scroll top-to-bottom in
...
Rationale: recruiters give portfolios one linear pass. A single governed page with a
...
| 7 | Engineering Timeline | P2 | Collapsed list; each checkpoint expandable |
...
- **Visual centerpiece:** the **live governed identity strip** — the real runtime
  ...
  passed."** This is the one thing on the hero that a technical recruiter cannot
...
**What is interactive:** the recruiter may switch between the small set of **real**
...
**The spine:** a single directed evidence chain the recruiter can traverse:
...
3. **The re-check invitation** — the exact artifact path in the repo and, where relevant,
   the command that regenerates or re-verifies it. The recruiter can copy a hash and
...
- **Governed hashes** — a wall of SHA-256s that all resolve to real files; the recruiter
...
**Design:** a single **directed flow** the recruiter can trace with their eye and expand
node by node:
...
chain, not a verdict*. The architecture experience must let the recruiter follow that
...
understanding, not a gap.* The UI must make a recruiter think "this person knows exactly
...
1. **"Verify it yourself."** The recruiter copies a real SHA-256 from the page and
...
2. **Raw score ≠ trust.** The verdict shows a real raw anomaly measure beside a panel that
   honestly reads *"calibrated confidence — not yet demonstrated."* The recruiter grasps,
   in one glance, the distinction most ML projects never even acknowledge.
3. **Machine-precision runtime equivalence.** The moment the recruiter sees that the ONNX
...
Each moment is a *fact the recruiter can check*, not an effect they watch.
...
path carries only the real learned signal. Every link is a hash the recruiter can check.
...
in the same calm voice it uses for its proofs. The story it leaves the recruiter with is
...
**Rationale.** The recruiter experience is fully specified at the architectural level: a
...
- Evidence envelope grounded in: `docs/evidence/KALIBRA_SCIENTIFIC_EVALUATION_EVIDENCE_v1.0.md`,
...
- Prior UX findings: `docs/checkpoints/KALIBRA_PORTFOLIO_UX_STACK_AND_PROTOTYPE_REVIEW_v1.0.md`.
- Thesis and domain vocabulary: `docs/KALIBRA_CLAUDE_DESIGN_BRIEF_v1.0.md`.

**Next natural step:** review this persisted Portfolio UX Architecture before starting
the Claude Design visual redesign.
