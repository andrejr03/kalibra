# VisA Acquisition and Governance
...
No step in this list is executed by this record. It defines the order the future
...
- *No directory is created by this record.*
...
- **Future upgrade policy** — if upstream ever republishes (new dated archive, DOI, or
  ...
  reproducible. Migration of the evaluation to a new version is a separate approved
  decision.
...
harness, and no governance approval document. It does not present the placeholder model's
...
Only afterwards approve the governed acquisition sprint.
```

No governing document is updated in this task. The persisted record must be
