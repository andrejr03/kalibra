# Kalibra C-1 — Dataset Selection Closure & Governed Acquisition Checkpoint v1.0

**Status:** Recorded decision checkpoint — no sprint authorized
**Date:** 2026-07-05
**Repository baseline HEAD:** `1b723e1 docs: persist ml capability checkpoints`
**Branch:** `codex/initial-engineering-skeleton`

## About This Document

This document persists the C-1 Dataset Selection Closure decision. It records the
basis for moving the repository's dataset state from `DEFER` to `SELECTED — VisA`.
It does **not** update the Dataset Selection ADR, download any dataset, implement
code, or authorize training. It is a decision record only. It does not present any
model behavior as a scientific result, claims no real defect detection, and makes no
product-readiness or benchmark claim.
...
- This checkpoint records the decision basis for moving from `DEFER` to
...
image-alignment criterion), and this checkpoint is the authorized venue to make the
...
No domain-of-record claim authorized.
```
...
- The Data Strategy Decision Memo required **explicit acceptance that VisA is a
  governed proxy** rather than a direct domain match; this checkpoint makes that
  acceptance explicit.
...
- **No cast-aluminium / machined-component claim** is authorized.
- **No real defect-detection claim** is authorized.
- **No product-readiness claim** is authorized.
...
- **No model training** is authorized by this checkpoint.
...
That ADR update is a separate, authorized documentation action and is **not**
performed by this checkpoint. See the C-1 strategy context in
[KALIBRA_ML_CAPABILITY_ENGINEERING_STRATEGY_CHECKPOINT_v1.0.md](KALIBRA_ML_CAPABILITY_ENGINEERING_STRATEGY_CHECKPOINT_v1.0.md).
