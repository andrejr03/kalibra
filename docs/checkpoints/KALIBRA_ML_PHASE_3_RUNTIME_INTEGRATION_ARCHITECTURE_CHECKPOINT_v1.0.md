# Kalibra ML Phase 3 Runtime Integration Architecture Checkpoint v1.0

**Status:** Recorded — phase-opening architecture review (review only; not an implementation, not an authorization, not a roadmap update)
**Date:** 2026-07-06
...
This document persists the **ML Phase 3 Runtime Integration Architecture Checkpoint** as a
versioned checkpoint following the repository checkpoint convention. It is the **formal
opening checkpoint of ML Phase 3** and establishes the architectural objective, scope, and
engineering direction of the phase.

It is **not** an implementation — it writes no code, exports no model, and wires no
provider. It is **not** an authorization — it authorizes no sprint, no ONNX export, no
runtime integration, no calibration, and no product-facing capability. It is **not** a
roadmap modification — it modifies no ADR, no Strategy, no Implementation Authorization,
and no normative document. It records an architecture review and a readiness decision only.

It takes the tag `ml-phase-2-complete` (HEAD `a9743b4`) as the official baseline and the
[ML Phase 2 Closure Review](KALIBRA_ML_PHASE_2_CLOSURE_REVIEW_v1.0.md) as the closing
state of the prior phase. Throughout, **must**, **must not**, **complete**, and **not yet
demonstrated** carry the binding sense established across the ML Phase 2 documents.
...
The closure review's central finding is therefore verified against code: **the real learned
...
capability classes the Architecture & Capability Checkpoint convention requires.
...
The logical order only. **This authorizes nothing** — each item requires its own
authorization checkpoint before implementation.
...
own authorization): multi-seed characterization (scientific-debt #2) and calibration / Trust
...
8. **Full completion evidence persisted** under the AGENTS.md workflow (authorization +
   completion checkpoint pair, evidence document).
...
phases are out of scope for this checkpoint and are not discussed here.)
...
- **Checkpoint persistence workflow (mandatory).** Review → persist in
  `docs/checkpoints/` → only then update ADRs/normative docs. Every Phase 3 capability gets
  its own persisted authorization + completion checkpoint pair; nothing lives only in chat.
- **Authorization workflow (mandatory).** AGENTS.md's Decision → Persist Checkpoint → Review
  → Authorization → Implementation → Implementation Review → Persist Completion Evidence,
  followed for **every** Phase 3 capability. This checkpoint is only the *opening* step of
  that workflow — it authorizes nothing.
...
- **Track C-number / roadmap divergence (carried lesson).** Phase 2's closure noted that the
  ...
  roadmap checkpoint when it redefines capability numbers or use a distinct series, so a
...
6. **The governance discipline that makes this trustworthy is intact and mandatory.** The
   checkpoint/authorization/evidence/replay workflow carried forward (§8) is exactly what
...
remains behind its own separate authorization gate. This checkpoint **opens** the phase; it
does **not** authorize any implementation within it.
...
- **No authorization granted.** No ONNX export, no runtime integration, no provider change, no
  placeholder retirement, no multi-seed work, no calibration, and no product-facing capability
  is authorized by this checkpoint.
- **No roadmap update.** No ADR, Strategy, Evaluation Strategy, or Implementation Authorization
  was modified.
...
```text
Review this persisted ML Phase 3 Runtime Integration Architecture Checkpoint before opening
the first ML Phase 3 authorization checkpoint.
```

If the first Phase 3 capability is opened, the logical first step (per §6, **not authorized
here**) is the governed ONNX export of the fitted PaDiM baseline, which requires its own
authorization checkpoint — reviewed and approved by the repository owner — before any
