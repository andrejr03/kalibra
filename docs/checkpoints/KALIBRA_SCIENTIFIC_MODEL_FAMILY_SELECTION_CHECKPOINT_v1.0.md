# Kalibra Scientific Model Family Selection Checkpoint v1.0

**Status:** Recorded — scientific architecture decision checkpoint (no sprint authorized)
**Date:** 2026-07-05
...
This document persists the decision on the first real ML model family to replace
Kalibra's placeholder identity model. It is a scientific architecture decision record
only. It authorizes no sprint, implements no code, and modifies no ML runtime,
provider, dataset, evaluation, or authorization logic. It does not present the
placeholder model's behavior as a scientific result, does not claim real defect
detection, and makes no product-readiness claim.
...
Review the persisted ML capability checkpoint documents before committing, then begin
