# ADR 002: ML Inference Framework
...
It does not approve implementation. It does not approve model loading,
...
- ML Phase 1 local-provider closure, which records that the record proves a
...
This decision does **not** approve model loading, ONNX session creation,
...
first runtime candidate to use if and when the implementation governance gate
is revised.
...
implementation governance approval is revised and the remaining dataset and evaluation
...
governance approval if the repository owner chooses to approve the next slice. That
revision must preserve the constraints in this ADR and must explicitly state
whether Sprint 1B is approved.
...
- The ML Phase 2 Implementation governance approval is updated by the repository owner
  to approve the next bounded implementation slice.
...
- How will evaluation distinguish runtime numerical differences from model
  quality differences?
- What exact restrictions will the Implementation governance approval place on Sprint
...
- the Implementation governance approval must be updated by the repository owner;
...
- no model loading, session creation, inference, benchmark claim, or downstream
  domain change is approved by this ADR.
