...
approve implementation, does not select or update the runtime framework, and does
not fix metrics — those remain governed by their own documents and by the
Implementation governance gate. A dataset decision here, even a selection, grants
...
- **Implementation governance approval** — fixed the final governance gate; its status is
  Deferred, and it requires an approved dataset as one precondition.
- **Scientific Model Family Selection record** — selected **PaDiM** as the first
  model family, adding an image-alignment criterion relevant to VisA and MPDD.
- **C-1 Dataset Selection Closure record** — closed the dataset-selection decision
...
governance investigations, first model-family decision, and C-1 closure record are
all now on the record. The Data Strategy Decision Memo named the explicit acceptance
required for selecting VisA as a governed proxy; the C-1 record records that
...
- **Governance readiness.** **Selected with governed-acquisition obligations.**
  ...
  completes the record at source. The C-1 record records that the remaining
...
- **Decision:** `Selected` — selected as the **governed proxy dataset** and
  **governance anchor** for the first Kalibra ML baseline. Selection does not mean
  acquisition is complete, and it approves no implementation, training, evaluation,
...
complete, and no implementation is approved.
...
- **PaDiM selection favours VisA because of image alignment.** PaDiM is
  position-sensitive; the recorded model-family record identifies MPDD pose
...
- This ADR approves **no domain-of-record claim**.
- This ADR approves **no defect-detection claim**.
- This ADR approves **no benchmark claim**.
- This ADR approves **no product claim**.
...
- **Implementation remains separately gated.** This ADR does not approve runtime,
...
- **Framework ADR, Scientific Architecture, Evaluation Strategy, and Implementation
  governance approval remain unmodified.** This ADR applies their governance posture; it
