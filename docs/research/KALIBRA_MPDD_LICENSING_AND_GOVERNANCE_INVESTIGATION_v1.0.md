...
This is a focused governance investigation of the **MPDD (Metal Parts Defect
Detection)** dataset. It exists solely to determine whether MPDD can satisfy
Kalibra's governance requirements. It implements no code, **modifies no ADR**,
authorizes no dataset selection, and discusses no framework.
...
which currently records **DEFER DATASET SELECTION** with MPDD marked *Deferred —
license type unconfirmed*.

**Verification convention.** A **Verified** statement is confirmed against an
official/primary source (the dataset's own repository, its authors' paper, or the
hosting institution) during this investigation (research conducted July 2026). An
**Observation** is an inference or secondary-source report and is **not** treated as
fact. No conclusion here rests on an assumed permission.

**Headline finding.** The single largest previously-open item — MPDD's license type —
is now **resolved and verified**: the official repository's `LICENSE` file is
**Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International
(CC BY-NC-SA 4.0)**. This advances the record but does **not** make MPDD governance-
ready; other governance items remain unresolved (§3, §7).
...
open. It does not modify that ADR, authorize selection, or recommend MPDD: on verified
