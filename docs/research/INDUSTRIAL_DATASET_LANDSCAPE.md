...
Dataset Strategy approval criteria and the Implementation governance gate, not by
this survey.
...
recommendation. **No licensing statement here approves use.**
...
- **Research use.** Most surveyed datasets permit research use, but "research use" is
  not the same as Kalibra's intended use and does not by itself approve anything.
...
still pass the Implementation governance gate before any framework-backed work
begins.
...
future Dataset Selection ADR. It selects no dataset, recommends none, and approves
